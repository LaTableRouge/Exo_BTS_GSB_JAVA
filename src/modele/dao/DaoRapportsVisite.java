...
public class DaoRapportsVisite implements DaoInterface<RapportsVisite, Integer> 
{
    ...
    catch (SQLException ex) 
    {
        ...
        // préparer la requête
        String requete = "SELECT RAPPORT_VISITE.*,MEDICAMENT.*,PRATICIEN.* FROM MEDICAMENT INNER JOIN OFFRIR ON MEDICAMENT.med_depotlegal = OFFRIR.med_depotlegal \n" +
                         "INNER JOIN RAPPORT_VISITE ON OFFRIR.rap_num = RAPPORT_VISITE.rap_num INNER JOIN PRATICIEN ON RAPPORT_VISITE.rap_num = PRATICIEN.pra_num";
    ...
    public int recupererDernierID() throws DaoException 
    {
        int maxId = -1;
        try {
            Jdbc.getInstance().connecter();
        ...
        // préparer la requête
        String requete = "SELECT max(RAP_NUM) as maxID FROM RAPPORT_VISITE";
        Jdbc.getInstance().getConnexion();
        try 
        {
            PreparedStatement ps = Jdbc.getInstance().getConnexion().prepareStatement(requete);
        ...
        {
            throw new modele.dao.DaoException("DaoCompteRendu::getOne : erreur requete DERNIEREID : " + e.getMessage());
        }
    ...
    private RapportsVisite chargerUnEnregistrement(ResultSet rs) throws DaoException 
    {
        try 
        {
            ...
            RapportsVisite RapportsVisite = new RapportsVisite(0,null,null,null,null,null,null);
            ...
            RapportsVisite.setOffre(rs.getString("MED_DEPOTLEGAL"));
